...
function CommentSection(props){
    ...
    const handleShowMoreButton = (childrenCommentArray) => {
        ...
        setTimeout(function() {
            inputEl.current.classList.add("shrink");
        }, 20);
    ...
    function Comment({ inputEl, item, rows, showmore, setRows, userData, storyID, setArtDataComments, handleShowMoreButton, handleReplyButton}) {
        ...
        return (
            ...
            <>
                ...
                <CommentDisplay ref={inputEl} className={"replies"} key={item.id + "commentDisplay"} showmore={showmore} item={item} id={item.id} >
                    ...
                    {/* <CommentBody style={{gridArea: "body", fontSize: "15px"}}>{item.body} this comment ID is {item.id} and its children array is </CommentBody> */}
                    ...
                    <BottomBarWrapper>
                        ...
                        <span style={{cursor: "pointer", marginLeft: "10px", fontSize: "10px", lineHeight: "40px"}} onClick={() => handleShowMoreButton(item.comments)}> {item.comments === undefined || item.comments.length == 0 ? "" : hideCommentsOrShowComments(item.comments)} </span>
...

                    {
                        ...
                        props.artDataComments.map( (c) => {
                            ...
                            return (
                                ...
                                <Comment key={c.id} item={c} userData={props.userData} storyID={props.artData.id} setArtDataComments={props.setArtDataComments}  handleShowMoreButton={handleShowMoreButton} handleReplyButton={handleReplyButton} rows={props.rows} setRows={props.setRows} showmore={props.showmore} inputEl={inputEl}/>
