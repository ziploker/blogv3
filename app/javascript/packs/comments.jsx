...
const hideCommentsOrShowComments = (childrenCommentArray, props) => {
...
const Comment = ({ props, item, rows, showmore, setRows, userData, storyID, setArtDataComments, handleShowMoreButton, handleReplyButton}) => {

    console.log("+++++++++++++++++++++++++++++++++++runit+++++++++++++++++++++++++++++++++++++++++++++++++++")

    // const addToRefs = (el) => {

    //     console.log("size b4 going in is ", allReplyRefs.current.length )
    //     console.log("in================= addToRefs")

    //     if (el && !allReplyRefs.current.includes(el)){
    //         console.log("inside================= addToRefs")
    //         console.log(el)
    ...
    const nestedComments = (item.comments || []).map(com => {
        ...
        return <Comment props={props} style={{border: "2px solid blue"}} key={com.id} item={com} type="child" userData={userData} storyID={storyID} setArtDataComments={setArtDataComments} handleShowMoreButton={handleShowMoreButton} handleReplyButton={handleReplyButton} rows={rows} setRows={setRows} showmore={showmore} />
    ...
    return (
        ...
        <>
            ...
            <CommentDisplay className={"replies"} key={item.id + "commentDisplay"} showmore={showmore} item={item} id={item.id} >
                ...
                <BottomBarWrapper>
                    ...
                    <span style={{cursor: "pointer", marginLeft: "10px", fontSize: "10px", lineHeight: "40px"}} onClick={() => handleShowMoreButton(item.comments)}> {item.comments === undefined || item.comments.length == 0 ? "" : hideCommentsOrShowComments(item.comments, props)} </span>
...
function CommentSection(props){
    ...
    //console.info(props.showMoreButtonRefs.current)
    ...
    console.log("size right after creating allReplyRefs is ", allReplyRefs.current.length )


    // useEffect((props) => {


    //     if (artDataComments.length == 0){
    //     axios.post("/blog/get_comments", {
    ...
    //           console.log("=========response of get_article_info ========= = " + response.inspect)
    ...
    //     //console.info("useseeffeecct")
    //    //console.log("mee" + allReplyRefs.current)

    //    //console.info(allReplyRefs.current[1])

    //     console.log("useEffect, the allRepliesRefs length is = ", allReplyRefs.current.length)

    //     allReplyRefs.current.map ( item => {

    //         console.log("allReplyRefs mapped in useEffect", item)
    //     })
    ...
    const handleShowMoreButton = (childrenCommentArray) => {
        ...
        allReplyRefs.current.map ( (item, i) => {
...

                    {
                        ...
                        props.artDataComments.map( (c) => {
                            ...
                            return (
                                ...
                                <Comment key={c.id} item={c} userData={props.userData} storyID={props.artData.id} setArtDataComments={props.setArtDataComments}  handleShowMoreButton={handleShowMoreButton} handleReplyButton={handleReplyButton} rows={props.rows} setRows={props.setRows} showmore={props.showmore} props={props}/>
